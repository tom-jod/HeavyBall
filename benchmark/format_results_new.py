...
def read_benchmark_results(file_path):
    ...
    if not table_match:
        raise ValueError("Table format not recognized. Make sure the markdown table has the correct format with header separators.")
...
def create_visual_matrix_normalized(success_total_matrix, df=None):
        ...
        for j, optimizer in enumerate(plot_optimizers):
            ...
            if task != "Avg. Optimizer" and optimizer != "Avg. Task":
                if task in success_total_matrix.index and optimizer in success_total_matrix.columns:
                    ...
                    # Get runtime and attempts if available
                    if df is not None and not df.empty:
                        task_df = df[(df["benchmark"].str.startswith(task + "-")) & 
                                    (df["optimizer"] == optimizer) & 
                                    (df["success"] == True)]
                        if not task_df.empty:
                            runtime_value = f"{task_df['runtime'].mean():.1f}s"
                            attempts_value = int(task_df['attempts'].mean())
            ...
            # Add text for success rate, runtime, and attempts
            brightness = sum(face_color[:3]) * 0.333
            text_color = 'white' if brightness < 0.6 else 'black'
            
            # Don't add runtime/attempts to summary cells
            if not is_summary:
                # Success rate at top
               # ax.text(j, i - 0.2, f"{value:.0f}%", ha='center', va='center', 
               #     color=text_color, fontsize=10, fontweight='bold')
                
                # Runtime in middle (only if there was success)
                if original_count > 0:
                    ax.text(j, i - 0.2, f"{runtime_value}", ha='center', va='center', 
                        color=text_color, fontsize=10, fontweight='bold')
                
                # Attempts at bottom
                ax.text(j, i + 0.2, f"{attempts_value}", ha='center', va='center', 
                    color=text_color, fontsize=10, fontweight='bold')
            else:
                # Just show the value for summary cells
...
def main(file: str = typer.Argument("benchmark_results.md")):
    ...
    if success_total_matrix.empty:
        ...
        return
    fig = create_visual_matrix_normalized(success_total_matrix, df)
    if fig is None:

        ...
        for i in range(self.steps // self.group):
            ...
            if self.test_loader != None:
                ...
                if (
                    ...
                    and len(self.best_test_accuracies) == self.steps // self.group
            ...
            if self.estimate_condition_number:
                with torch.backends.cudnn.flags(enabled=False):
                    ...
                    if estimate:
                        ...
                        # --- Save directory ---
                        save_dir = f"Condition_number_estimates/{self.timestamp}"
                        ...
                        plt.tight_layout()
                        plt.savefig(f"{save_dir}/step_{(i+1)*1000}.png", dpi=500)
                        plt.close(fig)
                        ...
                        condition_number = estimate_condition_number_full(
                            self.m, self.data, num_batches=1, loss_fn=self.loss_fn, weight_decay=weight_decay
                        )
                        condition_number_approx = estimate_effective_condition_number_reorth(
                            self.m,
                            self.data,
                            loss_fn=self.loss_fn,
                            weight_decay=weight_decay,
                        ...
                        # --- Save directory ---
                        save_dir = f"eigenspectrum_visualisation/{self.timestamp}"
                        os.makedirs(save_dir, exist_ok=True)
                        ...
                        if not os.path.exists(csv_file):
                            with open(csv_file, mode="w", newline="") as f:
                                ...
                                writer.writerow([
                                    ...
                                    "condition_number_abs_threshold",
                                    "pos_lambda_min",
                                    "pos_lambda_max",
                                    "neg_lambda_min",
                                    "neg_lambda_max",
                        ...
                        with open(csv_file, mode="a", newline="") as f:
                            ...
                            writer.writerow([
                                ...
                                condition_number["condition_number_abs_threshold"],
                                pos_lambda_min,
                                pos_lambda_max,
                                neg_lambda_min,
                                neg_lambda_max,
                        ...
                        # Full caculation of the eigenvalues of the Hessian
                        condition_number = estimate_condition_number_full(
                            self.m, self.data, num_batches=5, loss_fn=self.loss_fn, weight_decay=weight_decay
                        )
                        ...
                        # --- Save directory ---
                        save_dir = f"true_eigenspectrum/{self.timestamp}"
                        os.makedirs(save_dir, exist_ok=True)
            ...
            for j in range(self.group):
                ...
                if self.requires_prev_minibatch(o):
                    ...
                    inp, tgt = (
                        {"current": (curr_inp, curr_tgt), "previous": (prev_inp, prev_tgt)},
                        None,
                    )  # tgt set to None since it's in the dict
                ...
                def _prev_closure():
                    ...
                    if prev_model_params is None:
                        ...
                        prev_model_params = {
                            name: param.data.clone().detach() for name, param in self.m.named_parameters()
                        }
                        return _closure()
...
def trial(
    ...
    param_names = {
        ...
        2: "one_minus_beta2",
        3: "one_minus_beta3",
        4: "one_minus_shampoo_beta",
        5: "weight_decay",
        6: "epsilon",
        7: "warmup_factor",
        8: "dropout_rate",
        9: "label_smoothing",
        10: "one_minus_momentum",
        11: "use_momentum",
        12: "max_preconditioner_dim",
        13: "precondition_frequency",
        14: "start_preconditioning_step",
        15: "inv_root_override",
        16: "exponent_multiplier",
        17: "grafting_type",
        18: "grafting_epsilon",
        19: "use_normalized_grafting",
        20: "communication_dtype",
        21: "communicate_params",
        22: "use_cosine_decay",
        23: "use_nadam",
        24: "step_hint_factor",
        25: "beta2",
    ...
    if test_optimizer_implementation:
        ...
        sampler = AutoSampler(
            seed=0x123125,
            search_space={
                f"param_{i}": optuna.distributions.FloatDistribution(1e-3, 1e-3)
                if i in [1, 2, 3, 4]
                else optuna.distributions.FloatDistribution(0, 0)
                if i == 5
        ...
        def _optuna_objective(trial):
            ...
            trial_result = {
                ...
                "condition_number_variances": obj.condition_number_variances.copy()
                if hasattr(obj, "condition_number_variances")
                else [],
        ...
        def _optuna_objective(trial):
            ...
            try:
                ...
                pass
            # Instruct Optuna to minimize 1 - accuracy if using a non-deterministic tuning technique
            return 1 - best_test_accuracy
    ...
    if all_trial_results:
        most_accurate_trial = max(
            all_trial_results, key=lambda x: max(x["test_accuracies"]) if x["test_accuracies"] else 0
        )
    ...
    std_err_cond = (
        np.std(final_condition_numbers) / np.sqrt(len(final_condition_numbers)) if final_condition_numbers else 0
    )
    torch.cuda.synchronize()
    end_time = time.time()
...
def estimate_minibatch_variance_grid_search(
    model, data_fn, loss_fn=torch.nn.functional.cross_entropy, task_type="auto", timout=3600
):
    """
    An aproach designed to adapatively change the number of probes and samples to make the problem obtain the highest
    accuracy possible in a given time budget
    """
    ...
    for n in n_samples:
        result, reached_timout = estimate_minibatch_variance_detailed(
            model, data_fn, n_samples=n, loss_fn=loss_fn, task_type="auto", timout=timout
        )
        if reached_timout:
            return results[-1]
        else:
...
def estimate_minibatch_variance_detailed(
    model, data_fn, n_samples=100, loss_fn=torch.nn.functional.cross_entropy, task_type="auto", timout=3600
):
    """
    Enhanced minibatch variance estimation that works for both classification and regression, a more computationally
    feasible version comnpared to cycling through all minibatches
    """
    model.train()
    gradient_samples = []
    loss_samples = []
...
def estimate_effective_condition_number_reorth(
    ...
    cond_abs_arr, cond_pos_arr, cond_lanczos_arr = (
        np.array(cond_abs_list),
        np.array(cond_pos_list),
        np.array(cond_lanczos_list),
    )
    return {
        ...
        "lanczos_condition_number_variance": float(np.var(cond_lanczos_arr, ddof=1))
        if cond_lanczos_arr.size > 1
        else float("nan"),

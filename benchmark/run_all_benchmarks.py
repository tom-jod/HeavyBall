...
def run_benchmark(script, opt, steps, dtype, trials, seed, difficulty):
    ...
    mean_cond = last_match(r"mean_cond: ([0-9.e\-+]+)", output)
    std_err_cond = last_match(r"std_err_cond: ([0-9.e\-+]+)", output)
...
def plot_loss_curves(results, opt, output):
    ...
    for r in results:
        ...
        print(f'{r["name"]}, {r["mean_cond"]}, {r["std_err_cond"]}')
